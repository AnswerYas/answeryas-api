  ...
  User.findOne({unique_id}, function(err, user) {
    ...
    user.comparePassword(password, (err, isMatch) => {
      ...
      if (!isMatch) {
        return res
          .status(400)
          .send({code: 1003, message: 'password does not match'});
      }
...
router
  .route('/profile')
  .get(passport.authenticate('bearer', {session: false}), (req, res, next) => {
    const {unique_id, mobile, email, nickname, gender} = req.user;
    res.status(200).json({unique_id, mobile, email, nickname, gender});
  })
  .put(passport.authenticate('bearer', {session: false}), (req, res, next) => {
    const {mobile, email, nickname, gender} = req.body
    User.findOne({token: req.user.token}, (err, user) => {
      ...
      user.save(err => {
        ...
        res.status(200).json({mobile, email, nickname, gender, unique_id: user.unique_id});
      })
    })
  });
